...
namespace Math
{
    _H_PUNK_MATH_CONFIG mat4 ProjectionMatrix(float fov, float aspect, float zNear, float zFar);
    _H_PUNK_MATH_CONFIG mat4 OrthoMatrix(float width, float height, float bottom, float top);
    _H_PUNK_MATH_CONFIG mat4 Rotate(vec4 v);
    _H_PUNK_MATH_CONFIG mat4 Translate(vec3 pos);
    _H_PUNK_MATH_CONFIG mat4 Scale(vec3 scale);
    _H_PUNK_MATH_CONFIG mat4 TargetCamera(vec3 eye, vec3 pos, vec3 up);
    _H_PUNK_MATH_CONFIG mat4 FreeCamera(vec3 pos, vec3 dir, vec3 up);
	_H_PUNK_MATH_CONFIG mat4 RotationMatrixFromQuaternion(const quat& q);
    _H_PUNK_MATH_CONFIG mat3 NormalMatrixFromWorldView(const mat4& worldView);
