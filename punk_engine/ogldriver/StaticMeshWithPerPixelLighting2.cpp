...
#include "Os.h"
#include "StaticMeshWithPerPixelLighting2.h"

StaticMeshWithPerPixelLighting2::StaticMeshWithPerPixelLighting2(int vCount, int fCount, float *v, float *t, float *n, unsigned short *i) :
...
StaticMeshWithPerPixelLighting2::~StaticMeshWithPerPixelLighting2()
{
	delete[] m_vertex;
	delete[] m_texture;
	delete[] m_index;
...
void StaticMeshWithPerPixelLighting2::Draw()
{
	glPushMatrix();
...
void StaticMeshWithPerPixelLighting2::SetNormalizationMap(GLuint textureId)
{
	m_normalizationCubeMap = textureId;
}

void StaticMeshWithPerPixelLighting2::SetBumpMap(GLuint textureId)
{
	m_bumpMap = textureId;
}

void StaticMeshWithPerPixelLighting2::SetLightColor(float* color)
{
	memcpy(m_lightColor, color, sizeof(m_lightColor));
}

void StaticMeshWithPerPixelLighting2::SetAmbientColor(float* color)
{
	memcpy(m_ambientColor, color, sizeof(m_ambientColor));
}

void StaticMeshWithPerPixelLighting2::SetLightPos(float x, float y, float z)
{
...
StaticMeshWithPerPixelLighting2* StaticMeshWithPerPixelLighting2::CreateFromFile(const char *filename)
{
	FILE* f = 0;
	...
	fclose(f);	
	StaticMeshWithPerPixelLighting2* mesh = new StaticMeshWithPerPixelLighting2(vertexCount, faceCount, vertex, texture, normal, index);
