...
#include "Os.h"
#include "StaticMeshWithPerPixelLightingAndSelfShading.h"

StaticMeshWithPerPixelLightingAndSelfShading::StaticMeshWithPerPixelLightingAndSelfShading(int vCount, int fCount, float *v, float *t, float *n, unsigned short *i) :
...
StaticMeshWithPerPixelLightingAndSelfShading::~StaticMeshWithPerPixelLightingAndSelfShading()
{
	delete[] m_vertex;
	delete[] m_texture;
	delete[] m_index;
...
void StaticMeshWithPerPixelLightingAndSelfShading::Draw()
{
	glPushMatrix();
...
void StaticMeshWithPerPixelLightingAndSelfShading::SetNormalizationMap(GLuint textureId)
{
	m_normalizationCubeMap = textureId;
}

void StaticMeshWithPerPixelLightingAndSelfShading::SetBumpMap(GLuint textureId)
{
	m_bumpMap = textureId;
}

void StaticMeshWithPerPixelLightingAndSelfShading::SetLightColor(float* color)
{
	memcpy(m_lightColor, color, sizeof(m_lightColor));
}

void StaticMeshWithPerPixelLightingAndSelfShading::SetAmbientColor(float* color)
{
	memcpy(m_ambientColor, color, sizeof(m_ambientColor));
}

void StaticMeshWithPerPixelLightingAndSelfShading::SetLightPos(float x, float y, float z)
{
...
StaticMeshWithPerPixelLightingAndSelfShading* StaticMeshWithPerPixelLightingAndSelfShading::CreateFromFile(const char *filename)
{
	FILE* f = 0;
	...
	fclose(f);	
	StaticMeshWithPerPixelLightingAndSelfShading* mesh = new StaticMeshWithPerPixelLightingAndSelfShading(vertexCount, faceCount, vertex, texture, normal, index);
