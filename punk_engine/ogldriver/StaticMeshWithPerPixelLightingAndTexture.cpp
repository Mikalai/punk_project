...
#include "Os.h"
#include "StaticMeshWithPerPixelLightingAndTexture.h"

StaticMeshWithPerPixelLightingAndTexture::StaticMeshWithPerPixelLightingAndTexture(int vCount, int fCount, float *v, float *t, float *n, unsigned short *i) :
...
StaticMeshWithPerPixelLightingAndTexture::~StaticMeshWithPerPixelLightingAndTexture()
{
	delete[] m_vertex;
	delete[] m_texture;
	delete[] m_index;
...
void StaticMeshWithPerPixelLightingAndTexture::Draw()
{
	glPushMatrix();
...
void StaticMeshWithPerPixelLightingAndTexture::SetNormalizationMap(GLuint textureId)
{
	m_normalizationCubeMap = textureId;
}

void StaticMeshWithPerPixelLightingAndTexture::SetBumpMap(GLuint textureId)
{
	m_bumpMap = textureId;
}

void StaticMeshWithPerPixelLightingAndTexture::SetDecal(GLuint textureId)
{
	m_decal = textureId;
}

void StaticMeshWithPerPixelLightingAndTexture::SetLightPos(float x, float y, float z)
{
...
StaticMeshWithPerPixelLightingAndTexture* StaticMeshWithPerPixelLightingAndTexture::CreateFromFile(const char *filename)
{
	FILE* f = 0;
	...
	fclose(f);	
	StaticMeshWithPerPixelLightingAndTexture* mesh = new StaticMeshWithPerPixelLightingAndTexture(vertexCount, faceCount, vertex, texture, normal, index);
