...
#include "Os.h"
#include "StaticMeshWithPerPixelLightingSelfShadinAndDecalMap.h"

StaticMeshWithPerPixelLightingSelfShadingAndDecalMap::StaticMeshWithPerPixelLightingSelfShadingAndDecalMap(int vCount, int fCount, float *v, float *t, float *n, unsigned short *i) :
...
StaticMeshWithPerPixelLightingSelfShadingAndDecalMap::~StaticMeshWithPerPixelLightingSelfShadingAndDecalMap()
{
	delete[] m_vertex;
	delete[] m_texture;
	delete[] m_index;
...
void StaticMeshWithPerPixelLightingSelfShadingAndDecalMap::Draw()
{
	glPushMatrix();
	...
	glFinalCombinerInputNV(GL_VARIABLE_D_NV, GL_ZERO,
		GL_UNSIGNED_IDENTITY_NV, GL_RGB);
	glFinalCombinerInputNV(GL_VARIABLE_E_NV, GL_TEXTURE2, 
		GL_UNSIGNED_IDENTITY_NV, GL_RGB);
...
void StaticMeshWithPerPixelLightingSelfShadingAndDecalMap::SetNormalizationMap(GLuint textureId)
{
	m_normalizationCubeMap = textureId;
}

void StaticMeshWithPerPixelLightingSelfShadingAndDecalMap::SetBumpMap(GLuint textureId)
{
	m_bumpMap = textureId;
}

void StaticMeshWithPerPixelLightingSelfShadingAndDecalMap::SetDecalMap(GLuint texture)
{
	m_decalMap = texture;
}

void StaticMeshWithPerPixelLightingSelfShadingAndDecalMap::SetLightColor(float* color)
{
	memcpy(m_lightColor, color, sizeof(m_lightColor));
}

void StaticMeshWithPerPixelLightingSelfShadingAndDecalMap::SetAmbientColor(float* color)
{
	memcpy(m_ambientColor, color, sizeof(m_ambientColor));
}

void StaticMeshWithPerPixelLightingSelfShadingAndDecalMap::SetLightPos(float x, float y, float z)
{
...
StaticMeshWithPerPixelLightingSelfShadingAndDecalMap* StaticMeshWithPerPixelLightingSelfShadingAndDecalMap::CreateFromFile(const char *filename)
{
	FILE* f = 0;
	...
	fclose(f);	
	StaticMeshWithPerPixelLightingSelfShadingAndDecalMap* mesh = new StaticMeshWithPerPixelLightingSelfShadingAndDecalMap(vertexCount, faceCount, vertex, texture, normal, index);
