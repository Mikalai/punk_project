...
#include "Os.h"
#include "StaticMeshWithSpecular.h"

StaticMeshWithSpecular::StaticMeshWithSpecular(int vCount, int fCount, float *v, float *t, float *n, unsigned short *i) :
...
StaticMeshWithSpecular::~StaticMeshWithSpecular()
{
	delete[] m_vertex;
	delete[] m_texture;
	delete[] m_index;
...
void StaticMeshWithSpecular::Draw()
{
	glPushMatrix();
...
void StaticMeshWithSpecular::SetNormalizationMap(GLuint textureId)
{
	m_normalizationCubeMap = textureId;
}

void StaticMeshWithSpecular::SetBumpMap(GLuint textureId)
{
	m_bumpMap = textureId;
}

void StaticMeshWithSpecular::SetLightColor(const float* color)
{
	memcpy(m_lightColor, color, sizeof(m_lightColor));
}

void StaticMeshWithSpecular::SetAmbientColor(const float* color)
{
	memcpy(m_ambientColor, color, sizeof(m_ambientColor));
}

void StaticMeshWithSpecular::SetEye(const float* eye)
{
	memcpy(m_eye, eye, sizeof(m_eye));
}

void StaticMeshWithSpecular::SetLightPos(float x, float y, float z)
{
...
StaticMeshWithSpecular* StaticMeshWithSpecular::CreateFromFile(const char *filename)
{
	FILE* f = 0;
	...
	fclose(f);	
	StaticMeshWithSpecular* mesh = new StaticMeshWithSpecular(vertexCount, faceCount, vertex, texture, normal, index);
