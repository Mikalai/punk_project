...
#include "Os.h"
#include "StaticMeshWithSpecular2.h"

StaticMeshWithSpecular2::StaticMeshWithSpecular2(int vCount, int fCount, float *v, float *t, float *n, unsigned short *i) :
...
StaticMeshWithSpecular2::~StaticMeshWithSpecular2()
{
	delete[] m_vertex;
	delete[] m_texture;
	delete[] m_index;
...
void StaticMeshWithSpecular2::Draw()
{
	glPushMatrix();
...
void StaticMeshWithSpecular2::SetNormalizationMap(GLuint textureId)
{
	m_normalizationCubeMap = textureId;
}

void StaticMeshWithSpecular2::SetBumpMap(GLuint textureId)
{
	m_bumpMap = textureId;
}

void StaticMeshWithSpecular2::SetLightColor(const float* color)
{
	memcpy(m_lightColor, color, sizeof(m_lightColor));
}

void StaticMeshWithSpecular2::SetAmbientColor(const float* color)
{
	memcpy(m_ambientColor, color, sizeof(m_ambientColor));
}

void StaticMeshWithSpecular2::SetEye(const float* eye)
{
	memcpy(m_eye, eye, sizeof(m_eye));
}

void StaticMeshWithSpecular2::SetLightPos(float x, float y, float z)
{
...
StaticMeshWithSpecular2* StaticMeshWithSpecular2::CreateFromFile(const char *filename)
{
	FILE* f = 0;
	...
	fclose(f);	
	StaticMeshWithSpecular2* mesh = new StaticMeshWithSpecular2(vertexCount, faceCount, vertex, texture, normal, index);
