...
{	
	...
	m_object = Utility::ObjectManager::Instance()->Load(L"ground_floor.object");
...
void Building::Render(Utility::CameraRef camera, Utility::TerrainRef terrain)
{
	...
	Render(m_object, Math::mat4::CreateTranslate(1000, terrain->GetHeight(1000, 1000), 1000)*Math::mat4::CreateRotation(1, 0, 0, -Math::PI/2.0));
}

void Building::Render(Utility::ObjectRef object, const Math::mat4& parent)
{	
	Math::mat4 mat;
	...
	{
		Render(child.get(), parent*object->GetLocalMatrix()*mat);
	}
}