...
namespace Gpu
{
    ...
    {
        if (!m_texture)
            m_texture = m_video_driver->CreateTexture2D(width, height, ImageModule::IMAGE_FORMAT_RED, 0, false);
        else
    ...
    {        
        int length = m_video_driver->GetFontBuilder()->CalculateLength(m_text);
        if (length == 0)
            return 1;
    ...
    void TextSurface::Save(System::Buffer *buffer) const
    {
    }

    void TextSurface::Load(System::Buffer *buffer)
    {
    }
    ...
    const System::string& TextSurface::GetText() const
    {
        return m_text;
    }

    Texture2D* TextSurface::GetTexture()
    {
        if (m_need_update)
            RenderTextToTexture();
        return m_texture;
