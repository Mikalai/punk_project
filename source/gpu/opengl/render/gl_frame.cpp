...
namespace GPU
{
    ...
    void Frame::FrameImpl::SetRenderTarget(RenderTarget target, Texture2D* buffer)
    {
        if (target == RenderTarget::Texture2D && buffer == nullptr)
            throw System::PunkInvalidArgumentException(L"Texture2D target should have a texture buffer");
        Top()->m_render_target = target;
        Top()->m_render_buffer = buffer;
    }

    RenderTarget Frame::FrameImpl::GetRenderTarget()
    {
        return Top()->m_render_target;
    }

    void Frame::FrameImpl::SetClearColor(const Math::vec4& value)
    {
        Top()->m_clear_color = value;
    }

    const Math::vec4 Frame::FrameImpl::GetClearColor() const
    {
	...
	void Frame::FrameImpl::BeginRendering()
	{
        ...
        if (Top()->m_render_target == RenderTarget::None)
            throw System::PunkException(L"Render target is not specified");

        if (Top()->m_render_target == RenderTarget::BackBuffer)
            m_render_target.reset(new OpenGL::RenderTargetBackBuffer);

        if (Top()->m_render_target == RenderTarget::Texture2D)
        {
            m_render_target.reset(new OpenGL::RenderTargetTexture(Top()->m_render_buffer, nullptr));
        }

        m_render_target->SetClearColor(Top()->m_clear_color);
	...
	void Frame::FrameImpl::EnablePerVertexColor(bool value)
    {
    ...
    void Frame::SetRenderTarget(RenderTarget target, Texture2D *buffer)
    {
        impl->SetRenderTarget(target, buffer);
    }

    RenderTarget Frame::GetRenderTarget()
    {
    ...
    {
		return impl->EnablePerVertexColor(value);
    }
}
