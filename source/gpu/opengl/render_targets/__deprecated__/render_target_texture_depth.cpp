#include "render_target_texture_depth.h"
...
namespace Gpu
{
    namespace OpenGL
    {
        RenderTargetTextureDepth::RenderTargetTextureDepth(Texture2D *depth_buffer, VideoDriver* driver)
            : RenderTarget(driver)
            ...
            , m_fb{0}
        {
            ...
            GL_CALL(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, m_depth_buffer->impl->m_texture_id, 0));
        ...
        RenderTargetTextureDepth::~RenderTargetTextureDepth()
        {
            GL_CALL(glBindFramebuffer(GL_FRAMEBUFFER, 0));
            GL_CALL(glDeleteFramebuffers(1, &m_fb));
        }

        void RenderTargetTextureDepth::SetViewport(const Math::Rect& value)
        {
            RenderTarget::SetViewport(value);
        }

        void RenderTargetTextureDepth::Activate()
        {
            GL_CALL(glDepthFunc(GL_LEQUAL));
            GL_CALL(glBindFramebuffer(GL_FRAMEBUFFER, m_fb));
            GL_CALL(glDrawBuffer(GL_NONE));
            GL_CALL(glReadBuffer(GL_NONE));
            GL_CALL(glViewport(0, 0, m_depth_buffer->GetWidth(), m_depth_buffer->GetHeight()));
            Clear(false, true, false);
        }

        void RenderTargetTextureDepth::Deactivate()
        {
            GL_CALL(glBindFramebuffer(GL_FRAMEBUFFER, 0));
            GL_CALL(glDrawBuffer(GL_BACK));
            GL_CALL(glReadBuffer(GL_BACK));
        ...
        void RenderTargetTextureDepth::Clear(bool color, bool depth, bool stencil)
        {
        ...
        void RenderTargetTextureDepth::Check()
        {
