...
//			{
//				std::unique_ptr<SolidEntity> e(*type_finder.GetResult().begin());
//				m_character->RemoveChild(e.get());
//				std::unique_ptr<TaskEat> task(new TaskEat);
...
//						{
//							std::unique_ptr<TaskTake> task(new TaskTake(m_entity_to_goto));
//							m_character->AddTask(task.release());
...
//							std::unique_ptr<TaskGoTo> task(new TaskGoTo);
