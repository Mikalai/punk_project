...
//#include "../../gui/vertical_scrollbar.h"
//#include "../../gui/vertical_slider.h"
//#include "../../gui/edit_box.h"
...
namespace OpenGL
{
	...
	void GUIRender::RenderWidget(const GUI::Widget* widget)
	{
		...
		m_states.CurrentState()->Get().m_blend_operation = CoreState::BLEND_SRC_ONE_MINUS_ALPHA;
		m_tc->Bind();

		m_rc->Begin();		
		m_rc->BindParameters(m_states.CurrentState()->Get());
		QuadObject::Instance()->Bind(m_rc->GetRequiredAttributesSet());
