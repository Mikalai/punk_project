...
namespace OpenGL
{
	struct RenderContextLightImpl : public RenderContextImpl
	{
		...
		RenderContextLightImpl::RenderContextLightImpl(const RenderContextLightImpl& impl)
	...
	RenderContextLight::RenderContextLight()
		: RenderContext()		
	{
		impl_rc.reset(new RenderContextLightImpl());

		std::auto_ptr<Shader> vertex(new VertexShaderLight());
		std::auto_ptr<Shader> fragment(new FragmentShaderLight());
		SetVertexShader(vertex.release());
		SetFragmentShader(fragment.release());
		impl_rc->m_vertex_attributes = VERTEX_POSITION|VERTEX_NORMAL;
	}

	RenderContextLight::RenderContextLight(const RenderContextLight& rc)
		: RenderContext(rc)
	{}

	RenderContextLight& RenderContextLight::operator= (const RenderContextLight& rc)
	{
		RenderContext::operator = (rc);
		return *this;
	}

	void RenderContextLight::SetDiffuseColor(const Math::Vector4<float>& c)
	{
		static_cast<RenderContextLightImpl*>(impl_rc.get())->SetDiffuseColor(c);
	}

	void RenderContextLight::SetProjectionMatrix(const Math::Matrix4x4<float>& m)
	{
		static_cast<RenderContextLightImpl*>(impl_rc.get())->SetProjectionMatrix(m);
	}

	void RenderContextLight::SetViewMatrix(const Math::Matrix4x4<float>& m)
	{
		static_cast<RenderContextLightImpl*>(impl_rc.get())->SetViewMatrix(m);
	}

	void RenderContextLight::SetWorldMatrix(const Math::Matrix4x4<float>& m)
	{
		static_cast<RenderContextLightImpl*>(impl_rc.get())->SetWorldMatrix(m);
	}

	void RenderContextLight::SetAmbientColor(const Math::Vector4<float>& v)
	{
		static_cast<RenderContextLightImpl*>(impl_rc.get())->SetAmbientColor(v);
	}

	void RenderContextLight::SetSpecularColor(const Math::Vector4<float>& v)
	{
		static_cast<RenderContextLightImpl*>(impl_rc.get())->SetSpecularColor(v);
	}

	void RenderContextLight::SetSpecularPower(float v)
	{
		static_cast<RenderContextLightImpl*>(impl_rc.get())->SetSpecularPower(v);
	}
	
	void RenderContextLight::SetLightPosition(const Math::Vector3<float>& pos)
	{
		static_cast<RenderContextLightImpl*>(impl_rc.get())->SetLightPosition(pos);
	};
}