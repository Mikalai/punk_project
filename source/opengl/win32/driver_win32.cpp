...
#include "../render/render_target_texture.h"
#include "../render/render_target_back_buffer.h"
...
namespace OpenGL
{
	...
	void Driver::RenderFrame(Frame* frame)
	{
		frame->Begin();
		frame->End();
	}
	...
	void Driver::OnResize(System::Event* e)
	{
		...
		for each (auto rt in m_targets)
		{
			rt->SetViewport(0.0f, 0.0f, (float)event->width, (float)event->height);
		}
	}
