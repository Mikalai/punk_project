...
{	
	...
	m_object = Utility::ObjectManager::Instance()->Load(L"collision_0.object");

	Utility::PhysicalSimulator::Instance()->CreateDynamicShapeBody(*m_object);
	Utility::RigidBodyDesc desc;
	desc.m_mass = 1000.0;
	desc.m_shape = m_object->GetShapeBody();
	m_rb.reset(Utility::PhysicalSimulator::Instance()->CreateRigidBody(desc));
	m_rb->SetLocation(Math::mat4::CreateTranslate(1000, 250, 1000));
...
void Building::Render(Utility::CameraRef camera, Utility::TerrainRef terrain)
{
	...
	Render(m_object, m_rb->GetLocation()
