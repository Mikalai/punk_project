...
class Viewer
{
	...
	float angle;
	bool m_game;	
	std::auto_ptr<Render::GLDebugDrawer> m_bullet_render;
...
public:
	...
	{				
		...
		m_field->GetTerrain()->SetScale(1.0f);
		...
		m_bullet_render.reset(new Render::GLDebugDrawer);
		m_bullet_render->SetCamera(&m_camera);
		Utility::PhysicalSimulator::Instance()->SetRender(m_bullet_render->GetImpl());
	...
	{	
		if (m_game)
		{
			...
			Utility::PhysicalSimulator::Instance()->Update(delta);
		}
		...
		{			
			...
			Utility::PhysicalSimulator::Instance()->Draw();
		}
