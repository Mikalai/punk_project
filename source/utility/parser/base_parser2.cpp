...
namespace Utility
{
	...
	KeywordCode Parse(const System::string& word)
	{
		...
		throw System::PunkInvalidArgumentException(System::string::Format(L"Unknown keyword %s", word.Data()));		
	...
	bool ParseTextures(System::Buffer& buffer, std::map<System::string, std::vector<Math::Vector4<Math::vec2>>>& value)
	{
		...
		while (1)
		{
			...
			case WORD_TEXTURE_COORD:
				...
				break;
			default:
				throw System::PunkInvalidArgumentException(System::string::Format(L"Unexpected keyword %s", Keyword[index]));
			}
		}
		return true;
	}
