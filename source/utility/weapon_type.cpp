...
//		stream.read((char*)&m_backsight_type, sizeof(m_backsight_type));	//	index of the backsight
	...
	void WeaponType::MakeFromScheme(const System::string& name, const Scene& scene)
	{
		...
		if (!weapon_tranform)
		{
			out_error() << L"Can't find 'weapon_tranform' scene is not a weapon scheme" << std::endl;
			return;
		}

		{
			...
			if (!body_mesh)
			{
				out_error() << L"Can't find '*body_mesh' scene is not a weapon scheme" << std::endl;
				return;
			}
		...
		//
		{
			...
			else
			{
				...
				if (!backsight_mesh)
				{
					out_error() << L"Can't find '*backsight_mesh' though 'backsight_transform was found, scene is not a weapon scheme" << std::endl;
					return;
				}
		...
		//
		{
			...
			else
			{
				...
				if (!holder_mesh)
				{
					out_error() << L"Can't find '*holder_mesh' though 'holder_transform was found, scene is not a weapon scheme" << std::endl;
					return;
				}
		...
		//
		{
			...
			else
			{
				...
				if (!light_mesh)
				{
					out_error() << L"Can't find '*light_mesh' though 'holder_transform was found, scene is not a weapon scheme" << std::endl;
					return;
				}
