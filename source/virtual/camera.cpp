...
namespace Utility
{
		...
		m_base_height(0)
	{
		...
		{
			int res;
			if (conf.ReadOptionInt(L"mouse_sense", res))
			{
				m_mouse_sense = (float)res / 10000.0f;
			}			
		}
		else
		{
		...
		{
			int res;
			if (conf.ReadOptionInt(L"move_forward", res))
			{
				m_move_forward = res;
			}
		}
		...
		{
			int res;
			if (conf.ReadOptionInt(L"move_backward", res))
			{
				m_move_backward = res;
			}
		}
		...
		{
			int res;
			if (conf.ReadOptionInt(L"move_left", res))
			{
				m_move_left = res;
			}
		}
		...
		{
			int res;
			if (conf.ReadOptionInt(L"move_right", res))
			{
				m_move_right = res;
			}
		}
