...
namespace Utility
{
	...
	Object::~Object()
	{
		m_static_mesh.reset(0);
		m_skinned_mesh.reset(0);
		m_shape_body.reset(0);
	}
	...
	void Object::Load(std::istream& stream)
	{
		...
		if (flag)
		{
			m_light.reset(Light::LoadFromStream(stream));
		}
	...
	void Object::UpdateProxy(Object* object)
	{
		if (object->IsProxy())
		{
			Object* root = object;
			while (root->GetParent() != 0)
				root = root->GetParent();
			Object* proxy = root->GetChild(object->GetReferenceObjectName());
			if (!proxy)
			{
